<template>
  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="label" label="明细"></el-table-column>
      <el-table-column prop="value" label="明细值"></el-table-column>
    ...
    <el-dialog
      ...
      <el-form label-width="50px" size="mini" :model="editForm">
        ...
        <el-form-item label="标签">
          <el-input v-model="editForm.label" placeholder="请输入字典明细标签"/>
        </el-form-item>
        <el-form-item label="值">
          <el-input v-model="editForm.value" placeholder="请输入字典明细的值"/>
        </el-form-item>
        <el-form-item label="序号">
          <el-input type="number" v-model="editForm.seq" placeholder="请输入字典明细序号"/>
        </el-form-item>
...
<script>
import {getByType} from '@/api/dict'
import {deleteById, listData, save, update} from "@/api/dictItems"
...
export default {
  ...
  methods: {
    getList() {
      // 准备要获取数据了
      this.loading = true
      this.searchForm.dictType = this.dict.type
      // 发送请求查询页面需要的数据
    ...
    handleSubmit() {
      let ret;
      this.editForm.dictType = this.dict.type
      if (this.editForm.id) {
        // 编辑
  ...
  created() {
    ...
    // 查询数据
    let dictType = this.$route.params.type
    // 发送请求查询页面需要的数据
    getByType(dictType)
      .then(res => {
        this.dict = res.data
